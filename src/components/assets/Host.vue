<template>
  <div class="app-container">
    ...
    >
     ...
     <el-table-column label="操作"  v-if="has_permission('system_ansible_host_view|system_ansible_host_add|system_ansible_host_edit')" align="center" width="100" class-name="small-padding fixed-width">       
       
        <template slot-scope="{row}">
          <el-button    type="primary" size="mini" @click="handleUpdate(row)">
            修改
          </el-button>
          <el-button v-if="row.status!='deleted' && has_permission('system_ansible_host_del')""  size="mini" type="danger" @click="delHost(row)">
            删除
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    postList() {
      this.$http.post('/api/ansible/host/v1',this.temp).then(response => {
    ...
    delHost(row) {
        ...
        this.$http.delete('/api/ansible/host/v1',{data:row}).then(response =>{
          this.dialogFormVisible = false
          this.$notify({
            title: 'Success',
    ...
    updateHost() {
      ...
      _this52.$confirm('此操作将修改该条数据, 是否继续?', '提示', {
        ...
        this.$http.put(`/api/ansible/host/v1`,_this52.temp).then(response => {
          console.log(response)
