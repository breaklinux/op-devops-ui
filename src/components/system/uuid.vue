<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" v-if="has_permission('system_channel_uuid_view|system_channel_uuid_add|system_channel_uuid_edit')"  align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" size="mini" @click="handleUpdate(row)">
            修改
          </el-button>
          <el-button v-if="row.status!='deleted' && has_permission('system_channel_uuid_del') " size="mini" type="danger" @click="delHost(row)">
            删除
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
   ...
   getList() {
       this.$http.get('/api/channel/uuid/v1', {
    ...
    postList() {
      this.$http.post('/api/channel/uuid/v1',this.temp).then(response => {
    ...
    updateHost() {
      ...
      _this52.$confirm('此操作将修改该条数据, 是否继续?', '提示', {
        ...
        _this52.$http.put(`/api/channel/uuid/v1`,_this52.temp).then(response => {
    ...
    delHost(row) {    
      this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        type: 'warning'
         ...
         this.$http.delete('/api/channel/uuid/v1',{data:row}).then(response =>{
          this.dialogFormVisible = false
          this.$notify({
            title: 'Success',
